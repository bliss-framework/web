...
The Bliss Framework's [general coding guidelines](../coding-guidelines/index.md) describe application services in the abstract. C# on ASP.NET Core is the language they were originally written against, so the fit is direct: the [three-layer model](../coding-guidelines/layers-of-application.md) maps onto **Controllers → Managers → Providers** almost word for word. What this page adds is a concrete, opinionated ASP.NET dialect — the `UserContext` object passed as the first argument to every business call, an autogenerated database provider layer (`db-gen`), the `ResponseModel<T>` envelope at the I/O boundary, and the conventions that keep a large ASP.NET solution navigable.

Read the [general coding guidelines](../coding-guidelines/index.md) and [general naming conventions](../coding-guidelines/general-naming-conventions.md) first — everything here builds on them. We assume Microsoft's [C# coding conventions](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/coding-style/coding-conventions) and [framework naming guidelines](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/naming-guidelines) as the baseline; this page only covers where Bliss adds or constrains a rule. This section is illustrated throughout with an imaginary **DocumentHub** application (ASP.NET Core, PostgreSQL, Serilog, Quartz) — a running example to make the conventions concrete, not a real product.
...
| Identifiers | Varies | `PascalCase` for types/methods/properties, `camelCase` for locals/parameters/**fields** (no `_` prefix — see [naming](./methods-and-parameters.md#private-fields-plain-camelcase)) |
...
6. **Side layer purity** — `Models`, `Mappers`, and `Helpers` carry no dependency on controllers, on the DI container, or on `HttpContext`. They could be lifted into a class library tomorrow. `TwinPeaks.DocumentHub.Common` is exactly that library.
...
The example DocumentHub solution has three projects because it hits exactly the first driver — reuse across more than one app:

```
TwinPeaks.DocumentHub.sln
├── TwinPeaks.DocumentHub.Web/            # The application — I/O, Management, Providers, Side
...
│
├── TwinPeaks.DocumentHub.Database/       # Generated provider infrastructure
│   └── Generated/
...
│
└── TwinPeaks.DocumentHub.Common/         # Cross-cutting, framework-free utilities
...
Same idea in either shape: the layer **roles** and the dependency direction are identical — a `Feature/Documents/` folder still contains a thin controller, an orchestrating manager, and an atomic provider, and dependencies still flow I/O → Management → Providers. Only the *physical grouping* changes. Namespaces still mirror the folder path, so feature-first files land in `Company.Product.Web.Features.Documents` (see [naming — projects and namespaces](./layer-types.md#projects-and-namespaces)). `Models/` almost always becomes a subfolder inside the feature because a single subject spawns many request/detail/query models; the handful of files at the feature root are the controller, the manager, and the provider(s).
...
The `TwinPeaks.DocumentHub.Database` project is autogenerated by `db-gen` from the PostgreSQL function catalogue (Go templates + `db-gen.json` / `routines.json`). For each stored function it emits three artifacts, named mechanically from the function name: a `DbContext.<Fn>Async(...)` method, a result `Generated/Models/<Fn>Model` (one `[DbColumnMapping]` property per column), and a `Generated/Processors/<Fn>Processor` row parser. The exact SQL→C# name mapping is on the [Generated code](./methods-and-parameters.md#generated-code-see-postgresql) naming page.
...
| 2. Environment | `appsettings.{Environment}.json` (`Development`, `TwinPeaksRelease`, …) | Yes | What differs by environment but is the same for every deploy in it |
