...
Namespaces mirror the folder path exactly: a file in `KeenMate.DocumentHub.Web/Managers/` is in `namespace KeenMate.DocumentHub.Web.Managers`. Under a [feature-first layout](./index.md#layer-first-or-feature-first) the same rule applies — `Features/Documents/DocumentsManager.cs` is in `KeenMate.DocumentHub.Web.Features.Documents`. Don't invent a namespace that doesn't match the folder, and don't put two unrelated top-level types in one file.
